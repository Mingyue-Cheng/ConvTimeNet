...
def weights_init(mod):
	...
	if classname.find('Conv') != -1:
		...
		if mod.bias is not None: mod.bias.data.fill_(0.01)        
...
class SimplePatch(nn.Module):
	def __init__(self, in_channels, out_channels, seq_len, patch_size, stride=1, norm='batch', padding_tp=None, device='cuda:0'):
		super(SimplePatch, self).__init__()
		...
		self.patch_net = nn.Sequential(
				nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=self.ptw, stride=stride, padding=0), 
			).to(device).apply(weights_init)
  
		self.norm_tp = norm
		self.norm = nn.LayerNorm(out_channels) if norm == 'layer' else nn.BatchNorm1d(out_channels)
		
	def forward(self, X): # Input: (bs, window, channel)
		X = X.permute(0, 2, 1)
		X = F.pad(X, (self.l_pad, self.r_pad), mode='constant', value=0)
		X = self.patch_net(X)
...
class BoxCoder(nn.Module):
	def __init__(self, patch_count, patch_stride, patch_size, seq_len, channels, weights=(1., 1.), tanh=False, device='cuda:0'):
		super().__init__()
	...
	def meshgrid(self, boxes): # Input: # (bs, patch_count, 2)
		...
		xs = torch.nn.functional.interpolate(boxes, size=self.patch_size, mode='linear', align_corners=True)
...
class OffsetPredictor(nn.Module):
	def __init__(self, in_feats, patch_size, stride, act='gelu', mod=0):
		...
		if mod == 0:
			...
			in_1, in_2, in_3 = patch_size * in_feats, 2 * (patch_size * in_feats) // 3, (patch_size * in_feats) // 3
	...
	def forward(self, X):
		...
		if self.mod in [0,1]:
			...
			patch_X = F.unfold(X.unsqueeze(1), kernel_size=(self.in_feats, self.patch_size), stride=(1, self.stride))
...
class DeformablePatch(nn.Module):
	def __init__(self, in_feats, out_feats, seq_len, patch_size, stride, padding_tp=None, norm='batch', act='gelu', offset_mod=0):	 
		super(DeformablePatch, self).__init__()
		...
		self.box_coder = BoxCoder(self.patch_count, stride, patch_size, self.new_len, in_feats)
  
		# output layers
		self.output_conv = nn.Conv2d(1, self.out_feats, (self.in_feats, self.patch_size))
		self.norm_tp = norm
		self.output_act = get_activation_fn(act)
		self.norm = nn.LayerNorm(self.out_feats) if norm == 'layer' else nn.BatchNorm1d(self.out_feats)
	...
	def forward(self, X, return_bound=False): # Input: (bs, window, channel)
		X = X.permute(0, 2, 1)
		X = F.pad(X, (0, self.n_padding), mode='constant', value=0)
		...
		output = output.view(B, self.patch_count, self.in_feats, self.patch_size) # (B, patch_count, channel, patch_size)
...
if __name__ == '__main__':
	window, patchsize, feats, device = 10, 4, 38, 'cuda:0'
	X = torch.randn(64, window, feats).to(device)
	model = DeformablePatch(window, patchsize, feats).to(device)
	model(X)
